...
export class AvalancheService implements OnModuleInit {
	...
	constructor(
		...
		this.settings = {
            ws : this.configService.get<string>("network.avalanche.ws"),
            chainId : this.configService.get<number>("network.avalanche.chainId"),
            networkId : this.configService.get<number>("network.avalanche.networkId")
        }
	}
    ...
    async connect(): Promise<boolean> {
        ...
        if (this.settings) {
            ...
            if (this.web3) {
                ...
                if (isListening) {
                    console.log('Avalanche Network connection successful.');
                    return true;
                } else {
                    this.web3 = null;
                    throw new Error('Cannot listen to Avalanche Network Provider.');
                }
            } else {
                this.web3 = null;
